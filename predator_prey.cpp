...
{
	 if( (count(w, 'f') + count(w, 'c')) >= count(w, 'r') ) {    // coyotes and foxes eat rabbits
        return (new empty(row, col));
...
{
	if( count(w, 'c') > count(w, 'f')+1 ) {    // coyotes eat foxes
        return (new empty(row, col));
...
/** if there are too many coyotes or coyote is too old then coyote dies */
creature* coyote::next(world& w)
{
    if( count(w, 'c') > ((2*look+1)*(2*look+1))/2 + 1) { 
        return (new empty(row, col));            // too many coyotes
    }else if(age >= lifespan) {                  
        return (new empty(row, col));            // coyote is too old
    }else {
        return (new coyote(row, col, age + 1));     // coyote is 1 year older
    }
}
